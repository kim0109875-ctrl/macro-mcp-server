...
이 서버는 두 가지 역할을 동시에 합니다.
  ...
  2. 일반 REST API (/api/...) — "종목노트" 프론트엔드(index.html)가 fetch()로 바로
     ...
     GET /api/news                             - 뉴스·리포트 화면 전체 (내 종목/관심 종목/시장 뉴스, 리포트 컨센서스)
...
# 뉴스 제목 번역의 1차 수단은 아래 MyMemory REST 호출(requests로 직접 호출 — 비공식
# 스크레이핑 방식보다 차단될 가능성이 낮습니다). deep_translator(Google)는 MyMemory가
# 실패했을 때만 쓰는 2차 백업입니다 — 패키지가 없거나 막혀 있어도 조용히 건너뜁니다.
try:
...
# 뉴스 제목 한국어 번역
#
# yfinance가 물어다 주는 뉴스는 Barron's/Motley Fool 같은 영문 매체가 대부분이라 원문
# 그대로 노출하면 화면이 영어투성이가 됩니다. 무료/비공식 Google 번역 엔드포인트를 쓰되,
# 이 서비스가 막혀 있거나(방화벽/레이트리밋) 실패해도 절대 화면이 깨지지 않도록 — 실패한
# 제목은 그냥 원문 그대로 보여줍니다. 같은 제목을 반복해서 다시 번역하지 않도록 프로세스
# 메모리에 캐싱합니다(제목 자체는 개인정보가 아니라 메모리에 계속 들고 있어도 무방).
# ---------------------------------------------------------------------------
_TITLE_KO_CACHE: dict[str, str] = {}


def _mymemory_translate_ko(text: str) -> str | None:
    """무료 MyMemory REST API로 직접 번역합니다(API 키 불필요). 실패하면 None."""
    try:
        resp = requests.get(
...
def _translate_one_ko(text: str) -> str:
    """1차: MyMemory REST. 실패하면 2차로 deep_translator(Google)를 시도하고,
    둘 다 실패하면 원문을 그대로 돌려줍니다(화면이 깨지지 않도록)."""
    out = _mymemory_translate_ko(text)
    if out:
        return out
    if _translator is not None:
        try:
            out2 = _translator.translate(text)
            if out2:
                return out2
        except Exception:
            pass
...
def _translate_titles_ko(titles: list[str]) -> None:
    """titles에 있는 제목들을 번역해 _TITLE_KO_CACHE에 채워 넣습니다 (반환값 없음,
    호출부는 _TITLE_KO_CACHE.get(원문, 원문) 형태로 읽어 씁니다)."""
    todo = [t for t in set(titles) if t and t not in _TITLE_KO_CACHE]
    if not todo:
        return

    def _one(t: str):
        return t, _translate_one_ko(t)

    with ThreadPoolExecutor(max_workers=6) as ex:
        for orig, translated in ex.map(_one, todo):
            _TITLE_KO_CACHE[orig] = translated
...
# 야후 파이낸스는 국내(.KS/.KQ) 상장 종목의 longName/shortName을 거의 내려주지 않아서,
# 그대로 두면 종목상세 화면 종목명 자리에 티커 코드가 그대로 노출됩니다(실제로 겪은 버그).
# 보유/관심 목록에 있는 종목은 여기 적어둔 실제 한글 종목명을 항상 우선 사용합니다.
_DEMO_TICKER_NAMES = {h["ticker"]: h["name"] for h in DEMO_HOLDINGS}
_DEMO_TICKER_NAMES.update({w["ticker"]: w["name"] for w in DEMO_WATCHLIST})
...
def _fetch_stock_info(ticker: str) -> dict:
    try:
        ...
        name = _DEMO_TICKER_NAMES.get(ticker) or info.get("longName") or info.get("shortName") or ticker
        ...
        # 뉴스 제목을 한국어로 번역해서 내보냅니다 (원문은 영문 매체가 대부분이라 화면에
        # 그대로 노출하면 어색합니다). 번역이 실패하면 원문 제목을 그대로 씁니다.
        _translate_titles_ko([n["title"] for n in news_items])
        for n in news_items:
            n["title"] = _TITLE_KO_CACHE.get(n["title"], n["title"])

        return {
            "ticker": ticker, "name": name, "currency": currency,
            # exchange 코드도 .info가 실패하면 같이 비어버려서 화면에 시장 표시가 빈 채로
            # 나갑니다. 이 서비스가 다루는 미국 종목(NVDA/AAPL/TSLA/ASML)은 전부 NASDAQ
            # 상장이라 안전하게 기본값으로 씁니다 (다른 미국 거래소 종목을 다루게 되면
            # 이 가정은 다시 확인이 필요합니다).
            "market": _EXCHANGE_LABEL.get(info.get("exchange"), info.get("exchange")) or
                     ("KOSPI" if ticker.endswith(".KS") else
                      ("KOSDAQ" if ticker.endswith(".KQ") else "NASDAQ")),
...
def _compute_trend(candles: list[dict], quote: dict) -> dict:
    ...
    if last and last.get("c") is not None and last.get("ma20") is not None:
        ...
        if last["ma20"]:
            vs_ma20 = round((last["c"] - last["ma20"]) / last["ma20"] * 100, 2)
    ...
    return {
        ...
        "cumulativeRate": cumulative_rate, "upDays": up_days, "downDays": down_days,
        "vsMa20": vs_ma20,
    }


def _fetch_analyst_targets(ticker: str) -> dict:
    """.info에 목표주가가 비어 있을 때 쓰는 보조 조회 (yfinance의 analyst_price_targets).
    종목 상세에서만 부릅니다 — 대시보드는 보유 종목을 전부 조회하는데 여기서 종목마다 요청을
    하나씩 더 보내면 느려지고 레이트리밋에 걸리기 쉬워서, 상세 화면 한 종목에만 씁니다."""
    try:
    ...
    return {"mean": _pos(apt.get("mean")) or _pos(apt.get("median")),
            "high": _pos(apt.get("high")), "low": _pos(apt.get("low"))}
...
# 뉴스·리포트 화면용 시장 지수 (코스피/코스닥/S&P500). 개별 종목이 아니라 "시장" 탭에서
# 쓰는 참고용 지수라 changeRate 계산 없이 뉴스만 뽑아 씁니다.
...
def _fetch_index_news(ticker: str, label: str, limit: int = 3) -> list[dict]:
    """지수 티커의 최신 뉴스만 가볍게 뽑습니다 (가격/재무 정보는 필요 없어서
    _fetch_stock_info 전체를 타지 않고 news만 별도로 조회)."""
    try:
    ...
    for item in raw_news[:limit]:
        ...
        if title:
            items.append({
                "stockName": label, "changeRate": None, "title": title, "link": link,
                "source": source, "publishedAt": published, "impact": _classify_news_impact(title),
            })
    _translate_titles_ko([n["title"] for n in items])
    for n in items:
        n["title"] = _TITLE_KO_CACHE.get(n["title"], n["title"])
    return items


def _fetch_news_report() -> dict:
    """뉴스·리포트 화면 전체 데이터.
    - 내 종목/관심 종목: 보유·관심 종목 각각의 실시간 뉴스를 종목명/등락률과 함께 모읍니다.
    - 시장: 코스피/코스닥/S&P500 지수 뉴스 (개별 종목이 아니라 지수 라벨을 종목명 자리에 씁니다).
    - 공시: DART 같은 전자공시 API 연동이 이번 범위에 없어 빈 리스트를 정직하게 돌려줍니다
      (프론트에서 "아직 지원하지 않음" 안내로 처리 — 가짜 데이터를 만들어 채우지 않습니다).
    - 리포트: 애널리스트 컨센서스(목표주가) 스냅샷. Yahoo Finance는 "언제 목표주가가
      바뀌었는지" 이력은 제공하지 않아 changedAt 대신 "기준일"(조회 시점)로 표기합니다.
    - 요약: LLM 연동이 이번 범위에 없어 null을 돌려주고, 프론트에서 정직하게
      "준비 중" 상태로 보여줍니다 (그럴듯한 문구를 지어내지 않습니다).
    """
    with ThreadPoolExecutor(max_workers=8) as ex:
        holding_infos = list(ex.map(lambda h: _fetch_stock_info(h["ticker"]), DEMO_HOLDINGS))
        watchlist_infos = list(ex.map(lambda w: _fetch_stock_info(w["ticker"]), DEMO_WATCHLIST))
        market_news_lists = list(ex.map(lambda idx: _fetch_index_news(idx["ticker"], idx["label"]),
                                         _MARKET_INDEX_TICKERS))

    def _stock_news(names_and_infos, per_stock: int = 3):
        out = []
        for name, info in names_and_infos:
            if "error" in info:
                continue
            for n in info.get("news", [])[:per_stock]:
                out.append({
                    "stockName": name, "changeRate": info.get("change_pct"),
                    "title": n["title"], "link": n.get("link"), "source": n.get("source"),
                    "publishedAt": n.get("publishedAt"), "impact": n.get("impact"),
                })
        # 최신순 정렬 (publishedAt이 없는 항목은 뒤로)
        out.sort(key=lambda n: n.get("publishedAt") or "", reverse=True)
        return out

    my_stocks_news = _stock_news([(h["name"], info) for h, info in zip(DEMO_HOLDINGS, holding_infos)])
    watchlist_news = _stock_news([(w["name"], info) for w, info in zip(DEMO_WATCHLIST, watchlist_infos)])
    market_news = [n for lst in market_news_lists for n in lst]
    market_news.sort(key=lambda n: n.get("publishedAt") or "", reverse=True)

    today_label = _today_kst().strftime("%Y.%m.%d")
    reports = []
    for h, info in zip(DEMO_HOLDINGS, holding_infos):
        if "error" in info or not info.get("target_mean"):
            continue
        price = info.get("price")
        target = info.get("target_mean")
        upside_pct = round((target - price) / price * 100, 2) if price else None
        reports.append({
            "stockName": h["name"], "changeRate": info.get("change_pct"),
            "asOfDate": today_label, "targetMean": target,
            "targetHigh": info.get("target_high"), "targetLow": info.get("target_low"),
            "analystCount": info.get("analyst_count"), "currency": info.get("currency"),
            "upsidePct": upside_pct,
        })
    reports.sort(key=lambda r: r.get("analystCount") or 0, reverse=True)

    return {
        "myStocks": my_stocks_news,
        "watchlist": watchlist_news,
        "market": market_news,
        "disclosures": [],  # 공시: 연동된 데이터 소스 없음 (정직하게 빈 배열)
        "reports": reports,
        "summary": None,    # AI 요약: LLM 연동 없음 (정직하게 null)
        "asOfDate": today_label,
    }
...
def _fetch_stock_detail(ticker: str, range_key: str = "3m", interval: str = "1d") -> dict:
    ...
    # "최근 추세"와 당일 시가/고가/저가는 차트에서 무슨 봉(5분봉 등)·기간을 골랐는지와 상관없이
    # 항상 일봉 기준이어야 합니다. 예전에는 차트용 candles를 그대로 써서, 분봉을 고른 브라우저에서는
    # 추세 표의 날짜 자리에 시각(14:55)이 나오고 당일 시고저도 마지막 분봉 값으로 잘못 나왔습니다.
    # 일봉 3개월/1년이면 이미 받은 candles를 재사용하고, 그 외에만 일봉을 한 번 더 받습니다.
    if interval == "1d" and range_key in ("3m", "1y"):
        daily_candles = candles
    else:
        daily_candles = _fetch_candles(ticker, "3m", "1d")
    quote = {
        "price": base["price"], "change": base["change"], "changeRate": base["change_pct"],
        "open": daily_candles[-1]["o"] if daily_candles else None,
        "high": daily_candles[-1]["h"] if daily_candles else None,
        "low": daily_candles[-1]["l"] if daily_candles else None,
    ...
    trend = _compute_trend(daily_candles, quote) if daily_candles else \
        {"windowDays": 7, "verdict": "-", "strength": "-", "reasons": [], "daily": [],
         "cumulativeRate": None, "upDays": None, "downDays": None, "vsMa20": None}
    if not trend.get("daily"):
        trend["daily"] = _fetch_recent_moves(ticker)
    # 프론트가 추세 표의 전일 대비·특징(20일선 돌파, 거래량 급증 등)을 계산할 수 있도록
    # 표 7일 앞의 일봉까지 같이 내려줍니다 (차트용 candles는 분봉일 수 있어서 쓸 수 없음).
    trend["history"] = daily_candles[-30:]
    ...
    target_mean, target_high, target_low = base.get("target_mean"), base.get("target_high"), base.get("target_low")
    if not target_mean:
        alt = _fetch_analyst_targets(ticker)
        target_mean = alt.get("mean")
        target_high = target_high or alt.get("high")
        target_low = target_low or alt.get("low")
    consensus = None
    if target_mean:
        upside = ((target_mean - base["price"]) / base["price"] * 100) if base["price"] else None
        consensus = {
            "reportCount": base.get("analyst_count"), "targetAvg": target_mean,
            "targetHigh": target_high, "targetLow": target_low,
    ...
    for ev in events:
        ev["dDay"] = _compute_dday(ev["date"])
        ev.update(_importance_meta("high"))  # 개별 종목 실적발표는 상(high)로 취급 — 대시보드와 동일 기준

    # 종목별 실적 발표 일정만 쓰면 대부분 텅 빕니다 (야후 파이낸스가 국내 상장 종목의 실적
    # 캘린더를 거의 제공하지 않음). 대시보드 "다가오는 일정"과 같은 매크로 일정(FOMC/한국은행/
    # MSCI 리밸런싱 등)도 함께 보여주되, 이 종목 자체의 실적 발표가 있으면 그걸 우선합니다.
    macro_events = _collect_macro_events()
    _rank = {"high": 0, "medium": 1, "low": 2}
    combined = events + macro_events
    combined.sort(key=lambda e: (e["date"], 0 if e.get("type") == "stock" else 1, _rank.get(e.get("importance"), 3)))
    events = combined[:4]
...
async def api_news(request: Request) -> JSONResponse:
    return JSONResponse(_cached("news", 30, _fetch_news_report))
